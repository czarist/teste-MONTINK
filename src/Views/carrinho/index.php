        ...
        <tbody>
            <?php foreach ($itens as $item): ?>
                <tr>
                    ...
                    <td>
                        <input type="number" min="1" value="<?= $item['quantidade'] ?>" class="form-control quantidade-input"
                            data-id="<?= $item['produto_id'] ?>" data-variacao="<?= $item['variacao'] ?? '' ?>">
                    </td>
                    <td>R$ <?= number_format($item['preco'], 2, ',', '.') ?></td>
                    <td>R$ <?= number_format($item['preco'] * $item['quantidade'], 2, ',', '.') ?></td>
                    <td>
                        <button class="btn btn-danger btn-sm remover-item" data-id="<?= $item['produto_id'] ?>" data-variacao="<?= $item['variacao'] ?? '' ?>">Remover</button>
                    </td>
                </tr>
            <?php endforeach ?>
        </tbody>
...
<script>
    function salvarQuantidades() {
        document.querySelectorAll('.quantidade-input').forEach(input => {
            const produto_id = input.dataset.id;
    ...
    document.querySelectorAll('.remover-item').forEach(button => {
        button.addEventListener('click', function() {
            const produto_id = this.dataset.id;
    ...
    function validarCupom() {
        ...
        fetch('/api/cupom/validar', {
            ...
            .then(data => {
                ...
                document.getElementById('descontoLabel').innerText = `Desconto aplicado: ${data.percentual}% (-R$ ${data.valor_aplicado})`;

                const totalAtual = <?= $total ?>;
                const novoTotal = totalAtual - data.desconto;
                document.getElementById('totalLabel').innerText = `Total: R$ ${novoTotal.toFixed(2).replace('.', ',')}`;
            });
    }
</script>
